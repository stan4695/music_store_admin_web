...
<html>
	<head>
        ...
        <link href="styles/showClienti.css" rel="stylesheet">
	        ...
	        <main class="d-flex flex-column">
	        	<div class="main-contrast-overlay">
	        		<% if ("admin".equals(role)) {%>
			        	<form class="add-client d-flex justify-content-end" action="newClient.jsp" method="post">
				       	...
				       	<form class="table-form" method="post">
				    		<table class="table table-light table-striped">
						    	<tr>
						            ...
						            <th class="id">
						                <a href="showClienti.jsp?sort=id&order=<%= "id".equals(request.getParameter("sort")) && "asc".equals(request.getParameter("order")) ? "desc" : "asc" %>">
						            ...
						            <th>
						                <a href="showClienti.jsp?sort=nume&order=<%= "nume".equals(request.getParameter("sort")) && "asc".equals(request.getParameter("order")) ? "desc" : "asc" %>">
						            ...
						            <th>
						                <a href="showClienti.jsp?sort=email&order=<%= "email".equals(request.getParameter("sort")) && "asc".equals(request.getParameter("order")) ? "desc" : "asc" %>">
						            ...
						            <th>
						                <a href="showClienti.jsp?sort=telefon&order=<%= "telefon".equals(request.getParameter("sort")) && "asc".equals(request.getParameter("order")) ? "desc" : "asc" %>">
						                    <% if ("telefon".equals(request.getParameter("sort"))) { %>
						                        <i class="fa-solid fa-arrow-<%= "asc".equals(request.getParameter("order")) ? "up" : "down" %>"></i>
						                    <% } %>
						                    <b>Telefon</b>
						                </a>
						            ...
						            <th>
						                <a href="showClienti.jsp?sort=adresa&order=<%= "adresa".equals(request.getParameter("sort")) && "asc".equals(request.getParameter("order")) ? "desc" : "asc" %>">
						                    <% if ("adresa".equals(request.getParameter("sort"))) { %>
						                        <i class="fa-solid fa-arrow-<%= "asc".equals(request.getParameter("order")) ? "up" : "down" %>"></i>
						                    <% } %>
						                    <b>Adresa</b>
						                </a>
						        ...
						        <%
							        ...
							        List<String> validColumns = Arrays.asList("id", "nume", "email", "telefon", "adresa");
							        if (!validColumns.contains(sortColumn)) {
							            sortColumn = "id";
							        }
							        
						            ResultSet rs = jb.vedeTabela("tblClienti", sortColumn, sortOrder);
						            int x;
						            while (rs.next()) {
						                x = rs.getInt("id");
				    		...
				    		<% if ("admin".equals(role)) {%>
							    <p align="center">
							        <button type="submit" formaction="removeClient.jsp" class="button btn btn-danger"><i class="fa-solid fa-trash"></i>Șterge înregistrările</button>
							        <button type="submit" formaction="modifyClient.jsp" class="button btn btn-primary"><i class="fa-solid fa-pen-to-square"></i>Modifică înregistrările</button>
							    </p>
						    <%} %>
						</form>
					</div>
